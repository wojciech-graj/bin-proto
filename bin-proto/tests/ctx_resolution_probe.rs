...
impl BitEncode<()> for CtxSensitive {
    fn encode<W: BitWrite, E: Endianness>(&self, w: &mut W, _: &mut (), (): ()) -> Result<(), bin_proto::Error> {
        BitEncode::<(), ()>::encode::<_, E>(&0xAAu8, w, &mut (), ())
    }
}
impl BitEncode<CustomCtx> for CtxSensitive {
    fn encode<W: BitWrite, E: Endianness>(&self, w: &mut W, _: &mut CustomCtx, (): ()) -> Result<(), bin_proto::Error> {
        BitEncode::<(), ()>::encode::<_, E>(&0xBBu8, w, &mut (), ())
    }
}
impl BitDecode<()> for CtxSensitive {
    fn decode<R: BitRead, E: Endianness>(_: &mut R, _: &mut (), (): ()) -> Result<Self, bin_proto::Error> {
        Ok(Self)
    }
}
impl BitDecode<CustomCtx> for CtxSensitive {
    fn decode<R: BitRead, E: Endianness>(_: &mut R, _: &mut CustomCtx, (): ()) -> Result<Self, bin_proto::Error> {
        Ok(Self)
    }
}

// The user-reported scenario: inline bounds AND derive-generated bounds coexist.
...
fn generic_impl_uses_generic_ctx_not_unit_ctx() {
    ...
    // not BitEncode<()> (0xAA).
    let bytes = Pre { field: CtxSensitive }
