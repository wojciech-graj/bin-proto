macro_rules! impl_container_write {
    (
        ...
        impl<$($a,)? Ctx, Tag, T> $crate::ProtocolWrite<Ctx, Tag> for $ty<$($a,)? T>
        where
...
mod cell {
    ...
    impl<Ctx, Tag, T> ProtocolWrite<Ctx, Tag> for Cell<T>
    where
        T: ProtocolWrite<Ctx, Tag> + Copy,
    {
        fn write(
            &self,
            ...
            self.get().write(write, byte_order, ctx, tag)
        }
    }
