...
impl Simple {
    ...
    fn process_bytes(&mut self, bytes: &[u8], settings: &Settings) -> Result<(), Error> {
...
mod test {
    ...
    fn serialises_the_data_with_32bit_length_prefix() {
        ...
        transport
            .send_raw_packet(&mut buffer, &data, &Settings::default())
            .unwrap();
    ...
    fn successfully_deserializes_data_with_32bit_length_prefix() {
        ...
        let mut buffer = BitReader::endian(Cursor::new(&expected_data), BigEndian);

        transport
            .process_data(&mut buffer, &Settings::default())
            .unwrap();
